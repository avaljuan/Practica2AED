...
p_simple <- ggplot(data = ds , aes(x = MinTemp, y = MaxTemp))
p_disp <- p_simple + geom_point()
p_disp
...
rm(list=ls())
load("data/weatherAUS.RData")
grafico1 <- ggplot(data = ds, aes(x = Temp9am, y = Pressure9am)) + geom_point()

grafico1

grafico2 <- ggplot(data = ds, aes(x = Temp9am, y = Pressure9am, shape = RainToday)) + geom_point()

grafico2 

grafico3 <- ggplot(data = ds, aes(x = Temp9am, y = Pressure9am, color = RainToday)) + geom_point()

grafico3

grafico4 <- grafico3 + labs(title = "Probabilidad de lluvia según condiciones meteorológicas", subtitle = "Relación entre temperatura y presión atmosférica a las 9 AM", x = "Temperatura 9AM(ºC)", y = "Presión atmosférica 9AM(mb)") 
grafico4 + geom_point()
ggsave('figs/Ejercicio2.pdf')
```
Podemos observar que el gráfico con represetnación por colores según `RainToday` es mejor que el de la representación por forma.
...
iris.grafico1_disp <- iris.grafico1 + geom_point()
...
#7
iris.grafico5 <- ggplot(data = iris.tidy, mapping = aes(x = Species, y = Value, color = Part)) + geom_jitter() + facet_grid(.~Measure)
iris.grafico5
ggsave('figs/Ejercicio3.pdf') # se almacena el último gráfico generado
```
En las figuras donde disponemos vertical u horizaontalmente según `Part`, se comparan mejor los valores de las dos variables si se disponen en 2 columnas, ya que los valores se organizan de forma vertical.
...
grafico_1 <- ggplot(data = DatosEjer4, mapping = aes(x= Fecha, y = Ventas, color = Fabricante)) + geom_point()

grafico_1

grafico_2 <- ggplot(data = DatosEjer4, mapping = aes(x= Fecha, y = Ventas, color = Fabricante)) + geom_point()+ geom_line()

grafico_3 <- ggplot(data = DatosEjer4, mapping = aes(x= Fecha, y = Ventas, color = Fabricante)) + geom_point()+ geom_line(mapping=aes(linetype = Fabricante))

grafico_3

grafico_4 <- grafico_3 + ggtitle("Ventas mensuales de difernetes compañías tecnológicas")

grafico_4

ggsave("figs/Ejercicio4.pdf")
```
El gráfico 1 es válido para evaluar las ventas de cada fabricante, ya que se diferencian bien la evolución en el tiempo de las ventas para cada fabricante.
...
grafico_1 <- ggplot(data = ice.river.df, mapping = aes(x = time))+ geom_line(aes(y = flow.vat),color="red") + geom_line(aes(y = flow.jok), color = "blue")

grafico_1

grafico_2 <- grafico_1 + labs(title = "Caudales de los ríos islandeses Vatnsdalsa (azul) y Jokulsa Eystri (rojo)", x = "Tiempo (años)", y=TeX("$Caudal(m^3/s)$"))
grafico_2
...
grafico_1 <- ggplot(data = ds, aes(x = Year, y = MinTemp)) + geom_boxplot()

grafico_1

grafico_2 <- ggplot(data = ds, aes(x = Year, y = MaxTemp)) + geom_violin() + labs(title="Distribución de temperaturas máximas por año", x = "Año", y = "Temperatura(ºC)")

grafico_2

grafico_3 <-  ggplot(data = ds, aes(x = Year, y = MaxTemp)) + geom_violin(aes(fill = Year)) + labs(title="Distribución de temperaturas máximas por año", x = "Año", y = "Temperatura(ºC)")

grafico_3

grafico_4 <- grafico_3 + theme(legend.position = "none", axis.text.x = element_text(angle =45))
